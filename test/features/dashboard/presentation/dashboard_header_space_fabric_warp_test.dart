...
void main() {
  group('space-fabric Header metric warp contract', () {
    test('RED declares the third family and four append-only stable shader IDs',
        () async {
      ...
      expect(engine, contains("label: 'Térszövet torzítás'"));
    });
    ...
    test('RED samples the canonical palette only after metric source mapping '
      ...
      expect(field, contains('vec2 sourceUv = spaceFabricSourceUv(uv, uPhase)'));
    ...
    testWidgets('RED renders the third family as the sole active selector '
      ...
      expect(find.text('Térszövet torzítás'), findsOneWidget);
      controller.dispose();
    });

    test('RED exposes configuration-only Space Fabric diagnostic identities',
        () async {
      final engine = await File(
        'lib/features/dashboard/presentation/core_modes/dashboard_header_visual_engine.dart',
      ).readAsString();
