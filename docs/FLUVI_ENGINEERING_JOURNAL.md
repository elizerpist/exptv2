...
## 2026-09-23 — Color Lab vs physical app Header foreground mismatch

- **User physical feedback:** the Balance Header dark amount text in the physical Android app reads materially harsher / higher-contrast than the visually smoother dark text in the current Color Lab screenshot, despite the user selecting the dark/“Fekete” foreground option in the app tuner.
- **Current application evidence:** tooling graph commit `d6eb121ba5979afc5939e19758c579d6ebb0b901` explicitly indexes exact application source `65f33a4e388f6f796b77f31a5360e8f2f82a6f01`. That app commit is `feat(header): add per-mode palettes and foreground` and is the implementation under physical review.
- **PROVEN app foreground contract at 65f33a4:** `DashboardHeaderForegroundColor.black => Colors.black` and `white => Colors.white` in `dashboard_header_visual_engine.dart`. Balance Header amount rendering in `balance_dashboard_core_surface.dart` consumes `frame.foregroundTextColor` through `Theme.of(context).textTheme.titleMedium?.copyWith(... fontWeight: FontWeight.w700)`; there is no matching Color Lab text shadow or white edge stroke on that Flutter Text.
- **PROVEN Color Lab contract:** current `docs/prototypes/color_lab.html` blob `11a8111a328426711d5b6dea31a984a83b870bad` defines `--spendee-balance-ink: #14213a`, not literal black. Its `.common-header-value` uses that ink at `23px`, `font-weight: 850`, `letter-spacing: -.04em`, `text-shadow: var(--spendee-balance-ink-shadow)`, and `-webkit-text-stroke: .45px rgba(255,255,255,.20)` with `paint-order: stroke fill`. The lab body also uses `Inter, ui-sans-serif, system-ui, ...`, while the Flutter app does not declare a matching app-wide font family in `FluviApp` and therefore inherits Flutter/platform default typography.
- **Diagnosis:** the source-level rendering contracts are not equivalent. The implementation translated the UI control label “Fekete” into exact `#000000`, while the Color Lab’s dark foreground is a deep blue-charcoal `#14213a` plus white shadow/stroke softening. Therefore the physical contrast difference is expected even before accounting for Chrome-vs-Skia antialiasing. Renderer AA may still contribute to residual differences, but it is not required to explain the observed mismatch.
- **UNPROVEN:** the exact perceptual share attributable to font-family difference, CSS 850 vs Flutter w700, CSS white stroke/shadow, browser scaling, and Chrome vs Flutter/Skia rasterization on the user device.
- **MISSING EVIDENCE for a parity repair:** a deliberate product decision whether “Fekete” should mean Color Lab charcoal parity or literal black; an app-vs-lab side-by-side after matching the dark ink token first; only if residual mismatch remains, isolate font family/size/weight/letter-spacing and edge-softening one variable at a time. Do not change multiple typography variables blindly.
- Prompt-writer action: journal only, build-trigger-free `[skip ci]` commit. No Flutter application source, tests, graph, prototype, or build pipeline file is modified here.
