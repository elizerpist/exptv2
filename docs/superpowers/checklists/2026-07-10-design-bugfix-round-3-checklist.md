...
- Verification method: mirrored the existing `SlideUpMenuCard` veil-tap dismiss path for category picker and vendor sheets in both home and shell hosts; set the outside veil to cover the full app surface while the sheet card remains on top for inside interactions. Added widget tests that open category/vendor sheets, make pending selections, tap outside, assert the slide-down transform starts, assert the sheet disappears after the animation, and assert pending category/vendor filters are discarded. Updated header layout tests to assert header/type-pill taps outside an open category sheet cancel the sheet without passing the background action through. Ran full `test/transactions/category_menu_test.dart`, `test/transactions/header_layout_test.dart`, and targeted analyze for home/shell/category menu tests.
- Status: DONE
