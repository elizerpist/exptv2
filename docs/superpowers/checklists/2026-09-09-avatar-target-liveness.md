...
The6adeb42c CI prerequisite exposes one stale existing boundary expectation:416tests pass, but the interaction-performance boundary still requires the old literal30m runner budget. Before its small test-only correction, AVL16b verification scope is expanded to that existing boundary plus the complete local curated suite. Keep the baseline-specific15m assertion unchanged. The canonical parsed suite/SDK/shell/workflow ordering test remains the numeric policy owner; the existing general boundary must continue requiring a finite foreground TERM/kill watchdog around the entire drive process. No application or profile behavior change is needed for this correction.

The stale general boundary now requires a finite foreground TERM/kill watchdog without duplicating the numerical budget policy; the dedicated parsed ordering contract and baseline15m expectation remain unchanged. The entire local curated suite passes417tests in4m42, including all report, profile, interaction-performance boundary, application and Avatar composition cases. The original CI416/1red remains frozen. Only a test assertion changed after6adeb42; full exact-SHA CI/profile/APK/SCIP delivery remains pending.
