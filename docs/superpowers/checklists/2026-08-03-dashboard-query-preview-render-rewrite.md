...
| RWR-05 | Preview lane | rail/application | Every centered child projects synchronously from the in-memory batch index; no I/O during motion | motion isolation tests | PARTIAL — synchronous projection, root isolation, and seed-gated startup query boundary are green; full motion-I/O matrix remains |
| RWR-06 | Commit lane | query coordinator | Preview promotion is a no-op for an identical snapshot and does not reread | promotion tests | PARTIAL — store promotion, prewarm, and latest-wins lease-coordinator tests are green; end-to-end motion/lease isolation remains |
