...
| MYH-19 | CI runs 34759432562 and 34761546881 B frame-timing failures | Profile B FrameTiming sampler | The 12 ms / ≤1-missed-frame gate samples actual held-pointer amount previews only. It drains pre-existing engine timing batches, records twenty real pointer moves while either thumb remains down, and releases only after the preview sample; post-release canonical work remains separately verified but cannot be attributed to the preview frame sample. | Failed Android logs as RED evidence; focused harness/report test; final Android B profile | PARTIAL — local analyzer/report test pass; Android profile pending |
