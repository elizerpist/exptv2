...
| ID | Source | Code area | Acceptance condition | Verification | Status |
| --- | --- | --- | --- | --- | --- |
| FLB-1 | User task §5A, §6.3, §8.1 | `budget_category_avatar_artwork.dart` | Normal-avatar SVG and selected shell use one target-derived shadow-tone helper; no hardcoded pink shell cast shadow remains. | Unit/source contract using two target colors; APK visual check. | PARTIAL — implementation and focused contract are present; CI/APK visual evidence is pending. |
| FLB-2 | User task §5B, §6.2, §8.2 | Avatar SVG geometry | Centered-core viewport has the normal viewport dimensions and its source centre equals the sphere centre; normal rail retains its floor-biased full artwork viewport. | Numeric SVG geometry tests; APK visual check. | PARTIAL — numeric contract is implemented; CI/APK visual evidence is pending. |
| FLB-3 | User task §5C, §6.5, §8.3–8.4, §8.19 | Budget target catalog and rail | Handle 0 is a typed aggregate target; real categories follow authoritative inventory order; Expense Budget and Income Goal use exact reference titles, colors, gradients and prepared vectors. Zero categories still yields one cyclic target. | Target-domain/rail widget tests; direct reference inspection; APK visual check. | PARTIAL — code and focused tests are added; CI/APK evidence is pending. |
| FLB-4 | User task §5D, §8.5–8.9 | Core financial-limit domain, Room v5 | Immutable aggregate/category + sum/year/month key, scaled-100 integer amount, missing-versus-zero semantics, constraints, FK cascade, 4→5 migration, repository/use-case and revision-aware CRUD are present. | Kotlin unit/DAO/migration tests. | PARTIAL — implementation/tests added; Android Gradle CI is pending. |
| FLB-5 | User task §8.10–8.18 | Core query acquisition, Android bridge, runtime codec/cache | A query-independent exact-revision Budget snapshot is acquired through bounded set-based native work, transported in a versioned binary payload, decoded into dense O(1) target/period cells and cached by revision/year window. Query-only publication reuses the identical snapshot. | Native aggregation/codec tests; Flutter runtime/cache tests. | PARTIAL — implementation/tests added; full verification is pending. |
| FLB-6 | User task §8.16–8.18 | `DashboardPreparedRevisionBundle`, runtime publication | A dashboard bundle may publish only when index and Budget snapshot have the same revision; missing exact data fails closed without interaction-time repair I/O. | Bundle mismatch/fail-closed tests. | PARTIAL — implementation/tests added; full verification is pending. |
| FLB-7 | User task §8.20–8.24 | `DashboardBudgetPresentationController`, Budget header | Existing rail/time/direction semantic ticks bind `actual / limit` by dense O(1) lookup and rebuild only local Budget presentation. No database/bridge/query/LogBox work occurs on a tick. | Controller/widget/rebuild-isolation tests. | PARTIAL — implementation/tests added; full verification is pending. |
| FLB-8 | User task §8.26–8.28 | Native demo seed | Demo version and one seed transaction insert deterministic aggregate/category limits for both directions, SUM, all prepared years and all prepared months; under/equal/over examples exist per direction and plane. | Native demo-manifest coverage tests. | PARTIAL — implementation/tests added; Android Gradle CI is pending. |
| FLB-9 | User task §12–14 | Diagnostics, protected dashboard owners | Bounded financial-limit/target/header diagnostics exist; protected Query/LogBox/physics/geometry owners and zero-miss contracts remain unchanged. | Targeted protected suites; final diff review. | PARTIAL — final protected-suite/diff verification is pending. |
| FLB-10 | User task §15–16 and global delivery rule | Git/GitHub human delivery | Focused commits are pushed; the successful normal `lib/main.dart` APK is downloaded to `/storage/emulated/0/Download/fluvi` and SHA-256 verified. Human physical acceptance remains explicitly pending. | GitHub Actions/release artifact and local hash. | NOT DONE |
