...
| RCR-07 | User delivery rules | branch, CI, APK | All production changes are committed, pushed, verified through the exact SHA’s normal human diagnostic APK job, and the APK is downloaded with SHA-256. | Git, Actions and artifact verification. | PARTIAL — pending production commit, remote run and human APK retrieval. |
