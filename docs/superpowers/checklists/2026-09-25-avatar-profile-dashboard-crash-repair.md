...
| DEL-01 | Delivery rule | GitHub Actions + SCIP | Exact repair SHA has human APK evidence and separate exact-source graph. | CI/APK/tooling proof. | DONE — CI workflow `36103858468` passed test-core, test-flutter and human APK build for `df620bc...`; the downloaded v2-signed APK contains that SHA, and tooling `61f05571...` pins the exact source. |
| PHYS-01 | User evidence boundary | Android user | Repaired APK must be physically retested; previous avatar-switch validation is FAIL. | User only. | PENDING — USER ONLY; the pre-repair physical Avatar switch failed, while the exact repaired APK awaits device installation/retest. |
