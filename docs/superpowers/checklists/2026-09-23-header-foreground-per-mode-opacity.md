...
| HVAL-01 | Delivery validation | Focused Flutter suites, analyzer, Actions | Tests, formatter, analyzer and diff check have factual outcomes; a single final online APK is downloaded and hashed after all app work. | Command log / Actions evidence. | PARTIAL — local evidence complete; application commit, online workflow and APK pending. |
| HGRAPH-01 | Exact-source graph | Tooling branch | SCIP manifest source head equals final application SHA. | Tooling regeneration and deterministic hash. | NOT DONE — final application SHA not fixed yet. |
