...
| AUDIT-03 | §6–§7 | end-to-end render timeline | Child crossing is timed through selection, notification, build, layout, paint and presented frame, not only pointer swap. | Widget/profile recorder tests and physical report. | PARTIAL — instrumentation and schema tests pass; exact device timeline awaits PROF-02. |
...
| AUDIT-05 | §7 | density differential | Month/day empty/populated and year/month empty/populated first/2nd/5th/10th timelines identify what warms. | Cold-first fixture and physical report. | PARTIAL — 30× synthetic matrix and corrected cold-first/density timeline harness pass; exact profile/device artifacts are pending. |
| ROOT-01 | central question | causal report | The earliest evidenced mechanism common to `c35c680` and current code is named; scripted harness blind spots are documented. | Source comparison and pre-fix trace. | DONE |
...
| READY-03 | §4 | readiness barrier | `ready` requires the valid prepared index, semantic catalog, bounded current/adjacent payloads, indexed resources and the normal visible LogBox render surface to have presented. | Unit/widget tests. | DONE |
...
| READY-06 | §5, §15 | post-ready invariant | No rail-critical `FIRST_USE_WORK_STARTED`, cache miss, asset parse, VM build, row identity creation, controller or heavyweight viewport creation occurs after `ready`. | Hard diagnostic assertions and first-fling test. | DONE |
...
| LOG-07 | §20.6–7 | bounded cost | 94-entry/dense months create at most the bounded visible/overscan surface, and render cost does not scale with total entry count. | 0/2/4/9/24/94/dense performance matrix. | PARTIAL — structural bound and 30× matrix pass; profile/device render percentiles pending. |

## Resource, paint and cache isolation

| ID | Source | Intended code area | Acceptance condition | Verification method | Status |
|---|---|---|---|---|---|
| RES-01 | §4–§5 | category/icon/text resources | Deterministically preparable rail-critical assets, lookup tables, row identities and reusable formatter/resource objects are ready before interaction. | Readiness tests and zero first-use counters. | DONE |
| RES-02 | §5, §11 | vector renderer | The LogBox hot paint path performs no first SVG/vector decode and no per-row tint `saveLayer`; bounded prepared category imagery is reused. | Painter boundary test and paint diagnostics. | DONE |
...
| CACHE-01 | §13 | cache inventory | Data, prepared presentation, asset, text/layout, renderer/layer and viewport caches document owner/key/lifetime/warmup/eviction/max/miss behavior. | Root-cause report cache table. | DONE |
| CACHE-02 | §13 | critical miss guard | `RAIL_CRITICAL_CACHE_MISS` is a hard debug/test violation after `ready`; current/adjacent parent resources are bounded and explicitly pinned. | Unit/widget invariant tests. | DONE |
| MEM-01 | §14 | memory budget | Prepared index, viewport payloads, row VMs, asset cache, Flutter/native/graphics memory are measured and bounded; no full-widget preload. | Test estimates and profile RSS/memory artifact. | PARTIAL — bounds/byte estimates are exported; exact profile RSS pending. |
...
| PROF-01 | §7, §13, §16–§18 | profile validation | Exact-commit profile artifact includes first ten timelines, real/synthetic parity, UI/raster, LogBox render, GC/allocation/cache/first-use and memory fields. | GitHub profile workflow artifact. | NOT DONE |
...
| VERIFY-01 | global | local verification | Focused tests, full non-golden suite, analyze, boundary scan and `git diff --check` pass in Ubuntu proot. | Command output. | DONE — final 278/278 non-golden PASS; analyze `No issues found`; boundary and diff checks PASS. |
| VERIFY-02 | project workflow | online delivery | Final commit is pushed; exact-commit GitHub tests/profile/APK finish and APK is downloaded to `/storage/emulated/0/Download/fluvi`. | Actions run/artifact/hash. | NOT DONE |
| REPORT-01 | §21 | final report | All requested factual fields and every acceptance criterion are reported PASS/FAIL; any remaining FAIL/BLOCKED is not merge-ready. | Final report inspection. | NOT DONE |
