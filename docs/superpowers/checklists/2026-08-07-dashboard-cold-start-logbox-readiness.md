...
| AUDIT-03 | §6–§7 | end-to-end render timeline | Child crossing is timed through selection, notification, build, layout, paint and presented frame, not only pointer swap. | Widget/profile recorder tests and physical report. | PARTIAL — run `31171878871` exports the exact CustomPainter build/layout/paint/presented timeline; target-device timeline remains blocked by PROF-02. |
...
| AUDIT-05 | §7 | density differential | Month/day empty/populated and year/month empty/populated first/2nd/5th/10th timelines identify what warms. | Cold-first fixture and physical report. | PARTIAL — exact AOT first-ten artifact proves zero critical first-use/miss and identical synthetic inputs/endpoints; physical-device sampling remains blocked. |
...
| LOG-07 | §20.6–7 | bounded cost | 94-entry/dense months create at most the bounded visible/overscan surface, and render cost does not scale with total entry count. | 0/2/4/9/24/94/dense performance matrix. | PARTIAL — structural bound, 30× matrix and exact AOT maximum of four painted row slots pass; physical confirmation remains blocked. |
...
| RES-02 | §5, §11 | vector renderer | The LogBox hot paint path performs no first SVG/vector decode and no per-row tint `saveLayer`; bounded prepared category imagery is reused. | Painter boundary test and paint diagnostics. | DONE — one badge atlas and one group surface; each category glyph is an atlas-owned normalized vector picture, with no per-paint decode or tint layer. |
...
| PROF-01 | §7, §13, §16–§18 | profile validation | Exact-commit profile artifact includes first ten timelines, real/synthetic parity, UI/raster, LogBox render, GC/allocation/cache/first-use and memory fields. | GitHub profile workflow artifact. | DONE — run `31171878871` passed and uploaded exact first-ten, density, CustomPainter, GC/RSS, cache/first-use and memory evidence; unsupported per-gesture fields remain capability-marked. |
...
| VERIFY-02 | project workflow | online delivery | Final implementation commit is pushed; exact-commit GitHub tests/profile/APK finish and APK is downloaded to `/storage/emulated/0/Download/fluvi`. | Actions run/artifact/hash. | DONE — implementation `5b0e4523`, run `31171878871` all jobs PASS; `fluvi_5b0e452.apk` downloaded and ZIP/SHA-256 verified (`c85e1577…47943`). |
