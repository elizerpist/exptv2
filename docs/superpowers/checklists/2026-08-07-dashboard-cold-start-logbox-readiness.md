...
| PROF-01 | §7, §13, §16–§18 | profile validation | Exact-commit profile artifact includes first ten timelines, real/synthetic parity, UI/raster, LogBox render, GC/allocation/cache/first-use and memory fields. | GitHub profile workflow artifact. | PARTIAL — run 31160799254 produced the complete candidate artifact; final rerun is required for the corrected CustomPainter paint field. Unsupported per-gesture GC/allocation remains explicitly capability-marked. |
