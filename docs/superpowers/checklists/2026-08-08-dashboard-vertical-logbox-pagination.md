...
| VLOG-04 | §§5, 18 | rail scene cache/core | Rail scene always uses at most preview page size for 1/24/94/1k rows and never prepares vertical text on rail settle | frozen hashes + full rail matrix + profile gate | DONE (local matrix plus GitHub A–J profile gate green) |
...
| VLOG-14 | §35 | verification | Relevant non-golden suite and Ubuntu analysis pass, including no rail-settle vertical layout regression | proot Flutter commands + GitHub profile gate | DONE (317 Flutter tests, `flutter analyze`, and GitHub A–J profile gate green) |
| VLOG-15 | §35 | delivery | GitHub profile diagnostic APK builds, downloads to `/storage/emulated/0/Download/fluvi`, SHA-256/ZIP checks pass | Actions + local artifact check | DONE (`fluvi_534bf79.apk`, SHA-256 verified, ZIP integrity PASS) |
