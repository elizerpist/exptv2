...
| VFI-04 | Remove committed-page query handling/reply dependence on Android main thread | `MainActivity` query dispatch | Bounded committed page calls use the background query queue; reply/phase diagnostics identify queue/submission and Dart platform duration | Kotlin tests/build + physical diagnostics | PARTIAL — Dart transport test is green; local Gradle is unreliable under proot, so Android compile/test awaits online verification. |
| VFI-05 | Retain committed vertical pages around bidirectional motion by bytes, not a fixed page count | `CommittedLogViewportCache` | Small scopes stay hot when within a hard byte cap; large scopes retain visible/history/forward safety pages and evict non-hot LRU pages | Cache/paging tests | DONE — physical back-and-forth trace pending. |
| VFI-06 | Preserve exact identity and fail-closed rendering | Existing paging, cache, render owners | No stale/partial page, no changed physics/controller identities, no render-time layout, sequential cursors remain sequential | Regression suite + code inspection | DONE — physical trace pending. |
