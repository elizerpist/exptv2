...
| ARC-03 | §25 | surface/viewport layers | no new ticker, Query owner, range owner, scroll owner or gesture layer | boundary/source tests | DONE — one stable footer/range element, one temporal scroll owner and Header boundary/ticker tests are green. |
...
| FTR-03 | §17.2 | stable surface footer | range survives plane switch; exactly one slider/hit-test owner | mounted interaction test | DONE — `FTR-03` first reproduced the Day wrapper unmount; the stable passive host wrapper now keeps the same active RangeSlider element and held thumb values through Year → Sum → Month → Day. |
...
| NRG-01 | §35 | protected owners | preserve Time/Avatar, Query commit, snapping, LogBox, Budget, score math, fixture and Header visuals | full protected suites/diff review | DONE — Core production-parent 91/91, Mind/viewport/header focused 44/44, Header chart/tuner 26/26 and chart golden are green. |
...
| Performance and protected regressions | DONE |
| Analyzer/diff | DONE |

**BUILD GATE = CLOSED** only for the independently blocked native Android
coverage execution and final online delivery/SCIP evidence. No functional
