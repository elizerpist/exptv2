...
| ARC-03 | §25 | surface/viewport layers | no new ticker, Query owner, range owner, scroll owner or gesture layer | boundary/source tests + profile B | DONE — application `434276ac` moves the Day-only expansion recognizer inside the stable Mind surface. Online profile B reaches the later inherited FrameTiming gate without the former temporal content-gesture-region assertion. |
...
| SUM-03 | §§9.3,22 | admitted base boundary | full all-time coverage is source-proven; grid scrolls only when real years need it | coverage/performance test | DONE — source proves native all-filter scan retains all focus rows while Year/Month/Day frames alone are window-bounded; the clean online `test-core` lane for run `35285472445` executed the native suite successfully. |
...
| PRF-03 | §25 / online profile B | mode host | temporal Mind viewport has no outer content gesture region | mounted profile B + host widget test | DONE — run `35285472445` no longer fails the former `dashboard-core-mode-content-gesture-region` assertion; it proceeds to the independently inherited `frame_timing_headroom = null` profile gate. Focused `FTR-03` remains green. |
| NRG-01 | §35 | protected owners | preserve Time/Avatar, Query commit, snapping, LogBox, Budget, score math, fixture and Header visuals | full protected suites/diff review | DONE — online Flutter, clean Room/native bridge and human APK lanes are green for `434276ac` runtime source. Profile B validates the repaired topology then stops only at its pre-existing null FrameTiming harness gate; this failure is recorded separately, not greenwashed. |
| VAL-01 | §38 | Flutter/Core | focused projection, resolver, viewport, surface, score, Header and Query suites | exact Ubuntu commands | DONE — Ubuntu focused suites and analyzer are green; clean online `test-flutter` and `test-core` lanes are green in run `35285472445`. The local Termux AAPT2 pre-test limitation is superseded by that online native result. |
...
| DEL-01 | §§39–40 | Git/CI | atomic application commits, exact online CI/APK and separate journal commits | remote/APK evidence | DONE — `434276ac` plus journal `1da38093` are pushed. Run `35285472445` has green Flutter/core/human-APK jobs; profile B reaches only the known inherited null FrameTiming gate. `fluvi_HUMAN_DIAGNOSTIC_1da3809.apk` is downloaded and digest-verified. |
| DEL-02 | §40 | tooling branch | final exact-source SCIP manifest, raw hash and tooling tests | graph provenance | DONE — tooling `53c1f6da` is pushed with `manifest.source_head=434276ac`, raw index `0c7c77f7…`, and `dart test` 15/15. |
...
| Performance and protected regressions | DONE — profile B confirms the gesture-topology repair, then encounters only the independently inherited `frame_timing_headroom = null` harness failure. |
| Analyzer/diff | DONE |

**BUILD GATE = OPEN / DELIVERY COMPLETE** for the application feature. The
only non-green external evidence is the inherited profile-harness failure
`frame_timing_headroom = null`; it is not weakened, repaired speculatively or
reported as green.
