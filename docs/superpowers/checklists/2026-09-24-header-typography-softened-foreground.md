...
| HBUILD-02 | User §18 | GitHub Actions | One final workflow builds a human diagnostic APK from the final application SHA and it is downloaded, hashed and marker-verified. | Actions/release/APK checks. | PARTIAL — requires final app commit/push |
| HGRAPH-02 | User §§6, 18 | Tooling worktree | Separate SCIP tooling commit pins `manifest.source_head` to final app SHA and is deterministic. | Two generations + tooling tests. | PARTIAL — requires final app SHA |
