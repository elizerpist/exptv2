...
| B2P-006 | Migration: indexed/cached queries + final independent review | snapshot/cache + local log projection | One immutable snapshot prepares the complete active source scope (type, period, search, external category/merchant filters) and per-avatar aggregates/log indexes once per source revision; exact vendor identities remain collision-free; no legacy Balance log query or raw-record double scan occurs during interaction. | Task 8 independently inspected `a0703fb..9ce56d7`: structural source/query keys include immutable revision, avatar, exact vendor and normalized sorted scope; the B2 dashboard has zero matches for all three legacy Balance visible-log APIs. Fresh query/cache/selection/store matrix passed 78/78 (`00:18`), and the standalone 4,096-record direct-frame contract passed 1/1 (`00:32`). | DONE |
| B2P-007 | Existing V2 contract + final independent review | dashboard local query state + store adapter | A settled avatar updates a local primary query immediately then makes at most one shared-store acknowledgement; vendor remains a locally owned tertiary selection synchronized deliberately with the store; entering/leaving B2 and removing chips cannot leave avatar/card/legend/log state divergent. | Task 8 independently verified the single live `BudgetV2SelectionController`, local `BudgetV2QueryController`, acknowledged avatar/vendor ordering, abandoned-primary restoration and dashboard-only store acknowledgement. Fresh exact Task 7 route/diagnostic command passed 62/62 (`01:04`); the query/cache/selection/store matrix passed 78/78 (`00:18`). | DONE |
...
| B2P-009 | Migration: bounded diagnostics + final independent review | UI-free `budget_v2_interaction_diagnostics.dart` + scoped presentation policy | A bounded, sanitized terminal record contains only source revision/counts/index/commit duration; no standalone B2 direct-frame or vendor-ticker trace is emitted, and no merchant-derived value reaches `DebugConsole`. Legacy raw chart build traces are disabled only inside the standalone B2 context. | Task 8 independently inspected the bounded 16-record terminal queue and sanitized trace fields; production source has zero `_vendorDistributionKey`/`vendor_tick` matches. Fresh exact Task 7 route/diagnostic command passed 62/62 (`01:04`), including the live two-vendor privacy and 4,096-record diagnostic contracts. | DONE |
| B2P-010 | Global Flutter workflow | code/test suite | Targeted tests and full `flutter analyze` run successfully inside Ubuntu proot. | Fresh Task 8 Ubuntu/proot evidence: exact Task 7 command 62/62 (`01:04`), standalone 4,096-record contract 1/1 (`00:32`), query/cache/selection/ghost/store matrix 78/78 (`00:18`), and full `flutter analyze`: `No issues found! (ran in 46.9s)`. | DONE |
