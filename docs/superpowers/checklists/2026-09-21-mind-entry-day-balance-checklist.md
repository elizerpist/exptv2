...
| B2-HEADER | §14 Balance Header | Header detail seam | Existing Header material remains owner while it renders correct positive/negative net. | Balance surface widget test | DONE |
...
| B6-BOUNDS | §12/13 Balance bounds | Balance geometry/renderer | Centre is larger and has real unclipped layout/hit/semantic bounds; adjacent visual canvases deliberately do not overlap, so no contested paint order exists; exactly one selected semantic item. | Balance geometry/semantics test and collapsed CoreDashboard regression | DONE |
...
| V1-DELIVERY | §17–20 | commits/CI/APK/SCIP | Atomic application + journal commits, one final exact-SHA normal APK, and exact-source SCIP manifest are available. | Actions `35575616897`: Flutter/Core/APK PASS; known profile FrameTiming evidence failure explicitly separated. APK checksum/release and exact-source graph manifest verified. | DONE |
