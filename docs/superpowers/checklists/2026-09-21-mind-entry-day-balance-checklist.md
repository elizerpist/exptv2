...
| BX7-DELIVERY | user final build requirement | app commit, Actions, APK, SCIP, journal | All five requested features are in one app-code build; final graph matches exact app SHA and journal follows separately. | CI/APK/hash/manifest/git evidence | DONE — application `22f58adf`, one Actions build `35846311443`, APK/hash, exact-source SCIP tooling `53ce186f`, and separate journal evidence recorded; the long-standing out-of-scope Mind profile gate remains explicitly failed. |
