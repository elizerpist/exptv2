...
| VAL-PAGE-02 | §§17,19 | delivery | required focused suites, analyzer, fast/boundary checks, exact final CI/APK and exact-source SCIP are truthfully recorded | command/Actions/manifest evidence | PARTIAL — final app `0e97ce1` has focused 118-test, analyzer, 431-test fast and boundary evidence; run `35449748738` has dashboard-paths/test-flutter/test-core/Human APK PASS and the same inherited `frame_timing_headroom is invalid: null` profile failure as baseline. Its downloaded APK and deterministic exact-source SCIP are recorded; no global-green claim is made. |
