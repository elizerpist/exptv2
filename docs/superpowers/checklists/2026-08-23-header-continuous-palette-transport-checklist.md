...
| CPT-11 | Production profile remains within CI watchdog | shader palette sampling | The real 2-/3-probe Cool P(U) compiles with only its direct segments; the retained v3 bank keeps its complete float-index contract | RED/GREEN contract, full uniform-bank compile and successful A–J profile | PARTIAL — `ea7946a` still timed out; the source-level generic interval cascade is now removed and must be profiled on the next SHA |
| CPT-12 | Full CI and normal human APK delivery | GitHub Actions and `/storage/emulated/0/Download/fluvi` | Exact final SHA has all required CI jobs successful and its normal `lib/main.dart` human APK is downloaded with SHA-256 | GitHub Actions, local file check, SHA-256 | PARTIAL — `ea7946a` APK was downloaded but its A–J profile failed; the replacement SHA is not yet pushed |
