...
| CPT-07 | One shared Header clock; no data/UI hot-path regression | visual controller/backend | No new tickers, persistence, Query, LogBox, or rail work on phase ticks | Header/motion suites, protected dashboard suite and successful A–J profile | DONE |
...
| CPT-11 | Production profile remains within CI watchdog | shader palette sampling | The real 2-/3-probe Cool P(U) compiles with only its direct segments; the retained v3 bank keeps its complete float-index contract | RED/GREEN contract, full uniform-bank compile and successful A–J profile | DONE — `49e0a0e` completed every A–J scenario; profile job: 19m27s |
| CPT-12 | Full CI and normal human APK delivery | GitHub Actions and `/storage/emulated/0/Download/fluvi` | Exact final SHA has all required CI jobs successful and its normal `lib/main.dart` human APK is downloaded with SHA-256 | GitHub Actions, local file check, SHA-256 | DONE — `49e0a0e` CI succeeded; APK SHA-256 `20b7bccc835eeae2ea13905cea13223e74fb7b63231e1c8c5f9b14ea0cb6c2c5` |
