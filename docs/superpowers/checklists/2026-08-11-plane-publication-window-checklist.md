...
| PSP-06 | User §§22–24 | diagnostics/performance evidence | Diagnostics distinguish structural publication from rail interaction, with scene/row counts and input-to-commit timing. | Controller diagnostics implemented; physical-device latency capture still required after APK delivery | PARTIAL |
| PSP-07 | Global delivery rule | branch/CI/APK | Change is committed/pushed; exact GitHub human APK is monitored, downloaded and SHA verified. | GitHub Actions + local digest | NOT DONE |
