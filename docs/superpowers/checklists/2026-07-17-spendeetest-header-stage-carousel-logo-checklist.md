...
| BUILD-001 | User: one big build at end only | GitHub Actions | Only one final online build is triggered after all implementation and one commit/push. | Git/GitHub command log. | NOT DONE |
| BUILD-002 | User: download APK to `/storage/emulated/0/Download/spendee` | GitHub release/artifact download | Final APK is downloaded into the requested folder. | File existence check. | NOT DONE |
