...
| SCM-04 | Child index contract | Room service, bridge, Dart DTO | One grouped read yields child `SUM`, `COUNT`, canonical key, revision, and complete-index metadata | In-memory Room integration test | PARTIAL — test compiles; local ARM64 Robolectric cannot load Conscrypt. GitHub Ubuntu run pending. |
...
| SCM-10 | Delivery | repository and GitHub Actions | All checklist rows are DONE before push/build/download handoff | Checklist review and CI | PARTIAL — awaiting GitHub Ubuntu Room and APK workflow |
