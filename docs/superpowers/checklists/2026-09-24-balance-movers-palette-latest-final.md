...
| DEL-01 | User delivery instruction | App/tooling/docs delivery | One final application build after all three units, APK identity/hash, final exact-SHA SCIP, separate journal-only commit. | CI/APK/graph/Git evidence | PARTIAL — app/APK/exact graph complete; separate factual journal commit pending. |
