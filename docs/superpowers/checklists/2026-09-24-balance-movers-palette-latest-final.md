...
| REG-01 | All prompts/milestone | Existing systems | Header financial data, query/Summary, Mind/Budget, carousel mechanics, global geometry and Category/Partner contracts untouched. | Focused protected suites + diff audit | DONE — focused local evidence, clean diff and CI `test-core`/`test-flutter` pass; profile failure is the independently established Mind baseline. |
| DEL-01 | User delivery instruction | App/tooling/docs delivery | One final application build after all three units, APK identity/hash, final exact-SHA SCIP, separate journal-only commit. | CI/APK/graph/Git evidence | DONE — application `22fb148…`, exact APK/hash, final graph `bd734a94…`, and separate journal-only `dae299e2…` are pushed. |
