...
| PERF-02 | 6e96218 milestone / user scope lock | projection/chart | Toggle, pinch, tap and bar paint do no repository, Room, index, Query, Time or score work | Existing projection/widget work-bound tests | PARTIAL — source and focused work-bound tests are green; final CI/profile evidence pending |
| DELIV-03 | user workflow | GitHub/SCIP/journal | Application commits each have journal-only children; final exact app SHA has CI, human APK and SCIP | Git/Actions/artifact/graph audit | PARTIAL — application/journal commits, push, CI, APK and final graph remain pending |
