...
| PROFILE-IDLE-01 | Repeated aa26 profile-gate failures; current `CenteredCarouselController` source | Shared terminal motion lifecycle | A terminal `HoldScrollActivity` that becomes idle cannot leave the Time motion kernel in `drag`; it must publish one current settle without controller recreation, delay, timeout expansion or physics change | Red/green shared-carousel widget test plus Time/profile validation | PARTIAL — bounded Hold fallback and exact ScrollEnd handoff pass locally; the new-SHA profile run remains required under the software-EGL environment |
| PROFILE-IDLE-02 | Exact aca132 profile run + Flutter `ScrollPosition.beginActivity` source | Shared scroll-end lifecycle | A real latest-command `ScrollEndNotification` settles via one exact event-loop handoff after the framework installs Idle, without waiting for a renderer frame; stale/new-pointer/non-idle events cannot settle | Red/green shared and persistent production-parent notification regressions; exact-SHA profile CI | PARTIAL — production listener plus command-scoped microtask are implemented; shared, stale, held-command and persistent Time-rail regressions pass locally; exact-SHA profile CI remains pending |
| VALIDATE-01 | User §9 | Changed Dart/test surfaces | Red tests observed first; format, analyzer, focused, app, fast and presentation suites are compared against exact 9e baseline | Exact proot commands and normalized failures | PARTIAL — current change passes analyzer, shared motion (`+38`), Summary (`+48`), CoreDashboard (`+32`), Avatar/cache (`+112`), Mind/visible (`+78`), application (`+278`) and fast (`+291`); presentation is `+584 -19`, matching the documented clean-9e inherited signatures; final format and remote CI remain pending |
...
- **FAIL — normalized inherited:** `flutter test
  test/features/dashboard/presentation --reporter failures-only` — `+584 -19`.
